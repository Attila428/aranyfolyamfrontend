...
export default function Product({ allProduct, onAddToCart }) {
    return (
        <>
            {allProduct?.map(product => (
                <div className="container d-flex justify-content-center col-12 col-md-6 col-lg-4 col-xl-3" key={product.product_id}>
                    <div className="card bg-dark text-white rounded rounded-4">
                        <div className="p-3">
                            <img src={product.product_image} className="img-fluid rounded-4 rounded" alt={product.product_name} />
                        </div>
                        <div className="card-body">
                            ...
                            <div className="d-flex justify-content-between flex-md-row flex-column gap-3 mt-5">
                                ...
                                <div>
                                    <Button buttonClass={"btn btn-danger fw-bold fs-6"} content={"Kosárba"} onClick={() => onAddToCart(product)} />
                                </div>
                            </div>
                        </div>
                    </div>
